...
export function AppShell({
  ...
  return (
    <div className="min-h-dvh bg-bg">
      <aside className="pad-safe-top hidden w-56 flex-col border-r border-border bg-card md:fixed md:inset-y-0 md:left-0 md:z-20 md:flex">
            ...
            {NAV.map((item) => {
              ...
              return (
                <li key={item.to}>
                  <Link
                    to={item.to}
                    className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm text-muted data-[status=active]:bg-secondary data-[status=active]:text-foreground"
      ...
      <div className="md:pl-56">
        <header className="pad-safe-top pad-safe-x sticky top-0 z-20 border-b border-border/80 bg-bg/90 backdrop-blur-sm">
          <div className="mx-auto w-full max-w-5xl px-4 pb-3 sm:px-6 lg:px-8">
        ...
        <main
          className={cn(
            "mx-auto w-full px-4 pb-28 pt-5 sm:px-6 md:pb-12 lg:px-8 lg:pt-8",
      ...
      <nav
        className="pad-safe-bottom pad-safe-x fixed inset-x-0 bottom-0 z-20 border-t border-border bg-card/95 pt-2 backdrop-blur-sm md:hidden"
        aria-label="주요 메뉴"
      >
          ...
          {NAV.map((item) => {
            ...
            return (
              <li key={item.to}>
                <Link
                  to={item.to}
                  className="flex min-h-12 flex-col items-center justify-center gap-0.5 text-muted data-[status=active]:text-foreground"
                  activeOptions={{ exact: item.to === "/" }}
                >
                  <Icon className="size-5" strokeWidth={1.8} />
